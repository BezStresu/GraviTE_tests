...
// Podłącznie się przez putty żeby odczytywać komunikaty, baud_rate = 115200, numer portu COM do znalezienia w Device menager
int main(void)
{
  HAL_Init();
  SystemClock_Config();
  ...
  Max31865_init(&pt100, &hspi1, GPIOD, GPIO_PIN_14, 3, 50);		// przedostatni argument to rodzaj podłącznia, dla 3 jest to podłacznie na 3 przewody,
  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	// jeżeli podłączycie na 2 lub 4 to możecie tu wpisać dowolną liczbe
  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	// ostatni to częstotliwość filtru, można zmeienić na 60[Hz] ale zalecam zostawić jak jest
  while (1)
  {
  	HAL_Delay(5000);	// Pobiera i wysyła dane o temperaturze z zadanym okresem
  	float t;
...
// Pozostała część kodu poza opisaną wyżej to konfiguracja, gdybyście chcieli zmieniać coś w konfiguracji płytki za pomocą STM32MX to zalecam
// zapisać wcześniej osobno ten projekt gdyż kod MXu nadpisze ten
